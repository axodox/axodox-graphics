...
foreach ($platform in $platforms) {
  foreach ($config in $configurations) {
    ...
    MSBuild.exe .\Axodox.Graphics.sln -p:Configuration=$config -p:Platform=$platform -m:$coreCount -v:m
  }
}
...
$nuspec = [xml](Get-Content "$PSScriptRoot\Axodox.Graphics.nuspec")
...
$nuspec.Save("$PSScriptRoot\Axodox.Graphics.Patched.nuspec")

Write-Host 'Creating nuget package...' -ForegroundColor Magenta
.\Tools\nuget.exe pack .\Axodox.Graphics.Patched.nuspec -OutputDirectory .\Output
Remove-Item -Path '.\Axodox.Graphics.Patched.nuspec'